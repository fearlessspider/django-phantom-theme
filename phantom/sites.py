...
from phantom.forms import PhantomPasswordChangeForm
from phantom.models import Option
from phantom.views import OptionView, AccountView, EmailChangeView
...
class PhantomAdminDashboard(object):
    exclude = []
...
class PhantomAdminSite(AdminSite):

    top_menu_default_order = 3
    dashboard_class = PhantomAdminDashboard

    def __init__(self, *args, **kwargs):
        super(PhantomAdminSite, self).__init__(*args, **kwargs)
        self._top_menu = {}
        self._app_labels = {}
    ...
    def check_dependencies(self):
        ...
        :class:`phantom.middleware.PopupMiddleware` is installed
        and ``phantom`` is found in the INSTALLED_APPS setting **before**
        ``django.contrib.admin``.
        """
        super(PhantomAdminSite, self).check_dependencies()
        if not Option._meta.installed:  #@UndefinedVariable
            raise ImproperlyConfigured("Put 'phantom' in your "
                                       "INSTALLED_APPS setting in order to "
                                       "use the phantom application.")
        if settings.INSTALLED_APPS.index('phantom') > settings.INSTALLED_APPS.index('django.contrib.admin'):
            raise ImproperlyConfigured("Put 'phantom' before "
                                       "'django.contrib.admin' in your "
                                       "INSTALLED_APPS setting to use the "
                                       "phantom application")
        if not 'phantom.middleware.PopupMiddleware' in settings.MIDDLEWARE_CLASSES:
            raise ImproperlyConfigured("Put 'phantom.middleware.PopupMiddleware' "
                                       "in your MIDDLEWARE_CLASSES setting "
                                       "in order to use the phantom application.")
    ...
    def get_urls(self):
        ...
        up = patterns('',
                      url(r'^configuration-options/(?P<optionset_label>%s)/$' % '|'.join(_optionset_labels.keys()), wrap(OptionView.as_view()), name='optionset-label-options'),
                      url(r'^email-change/$', wrap(EmailChangeView.as_view()), name='email-change'),
                      url(r'^my-account/$', wrap(AccountView.as_view()), name='my-account'),
            )

        up += super(PhantomAdminSite, self).get_urls()
        return up
    ...
    def unregister_options(self, optionset_admin, delete_db_records=False):
        ...
        if optionset_label in _optionset_labels:
            ...
            if delete_db_records:
                Option.objects.filter(optionset_label=optionset_label).delete()
    ...
    def index(self, request, extra_context={}):
        """
        This index view implementation adds stats
    ...
    def i18n_javascript(self, request):
        ...
        js translation messages based on a phantom setting.
        """
    ...
    def password_change(self, request):
        ...
        defaults['password_change_form'] = PhantomPasswordChangeForm
